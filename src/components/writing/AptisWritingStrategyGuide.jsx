...
const FACT_ICONS = [PencilLine, ListChecks, Clock3];
...
export default function AptisWritingStrategyGuide() {
  ...
  return (
    <main className="aptis-strategy-page aptis-writing-strategy-page">
      <Seo
        title={`Aptis Writing Part ${guide.number}: ${guide.title} Strategy Guide | Seif Aptis Trainer`}
      ...
      <header className="aptis-strategy-hero">
        <p className="aptis-strategy-kicker">Aptis Writing Part {guide.number}</p>
        <h1>{guide.title}</h1>
      ...
      <section className="aptis-strategy-facts" aria-label={`Part ${guide.number} essentials`}>
        {guide.facts.map((fact, index) => {
          const FactIcon = FACT_ICONS[index] || PencilLine;
          return (
      ...
      {guide.wordCountNote ? (
        <aside className="aptis-strategy-reminder" aria-label="About the word count">
          <Gauge size={24} aria-hidden="true" />
          <div>
            <strong>About the word count</strong>
            <p>{guide.wordCountNote}</p>
          </div>
        </aside>
      ) : null}
