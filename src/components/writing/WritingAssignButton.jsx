...
export default function WritingAssignButton({
  user,
  activityId,
  activityLabel,
  routePath,
  ...
  useEffect(() => {
    ...
    async function loadStudents() {
      ...
      try {
        ...
        console.error("[WritingAssignButton] student load failed", error);
        if (alive) {
          setStudents([]);
  ...
  async function handleAssign() {
    ...
    if (!targetStudentIds.length) {
      toast("Choose at least one student for this writing task.");
      return;
    }
    ...
    try {
      ...
      toast("Writing task assigned.");
      close();
    } catch (error) {
      console.error("[WritingAssignButton] assignment failed", error);
      toast("Could not assign that writing task.");
    } finally {
      setSaving(false);
    }
  }
