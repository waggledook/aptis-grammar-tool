...
import openClozeSeedItems from "../../data/content/openCloze.json";
import {
  auth,
  clearHubOpenClozeMistakes,
  fetchHubOpenClozeFavourites,
  fetchHubOpenClozeMistakes,
  fetchOpenClozeItems,
  fetchSeenHubOpenClozeItemIds,
  logHubOpenClozeCompleted,
  logHubOpenClozeReviewLoaded,
  logHubOpenClozeStarted,
  recordHubOpenClozeMistake,
  removeHubOpenClozeFavourite,
  saveHubOpenClozeFavourite,
...
const REPORT_REASONS = [
  { value: "Incorrect answer", label: "The accepted answer is incorrect" },
  { value: "Missing valid answer", label: "A valid answer is missing" },
  { value: "Gap or sentence issue", label: "The gap or sentence is wrong" },
...
function stableIdFromChallenge(item) {
  const raw = `${item.gappedSentence || ""}`;
  let hash = 0;
  for (let i = 0; i < raw.length; i += 1) {
    hash = (hash << 5) - hash + raw.charCodeAt(i);
    hash |= 0;
  }
  return `oc_${Math.abs(hash)}`;
}

function prepareOpenClozeItem(item) {
  return {
    ...item,
...
export default function HubOpenClozeTrainer() {
    ...
    async function load() {
      ...
      try {
        const [data, favourites, seenIds] = await Promise.all([
          fetchOpenClozeItems().catch((error) => {
            console.warn("[HubOpenClozeTrainer] falling back to local open cloze bank", error);
            return [];
          }),
          auth.currentUser ? fetchHubOpenClozeFavourites() : Promise.resolve([]),
          auth.currentUser ? fetchSeenHubOpenClozeItemIds() : Promise.resolve([]),
        ]);

        if (!cancelled) {
          const fallbackItems = Array.isArray(openClozeSeedItems) ? openClozeSeedItems : [];
          const sourceItems = Array.isArray(data) && data.length ? data : fallbackItems;
          setItems(sourceItems.map(prepareOpenClozeItem));
          setFavouriteIds(new Set(favourites.map((item) => item.itemId)));
          setSeenItemIds(new Set(seenIds));
        }
      } catch (error) {
        console.error("[HubOpenClozeTrainer] load failed", error);
        if (!cancelled) toast("Could not load open cloze items.");
      } finally {
  ...
  function startGame() {
    if (!challengePool.length) {
      toast("No open cloze items match those filters.");
      return;
    }
  ...
  async function loadFavouriteChallenges() {
    try {
      const favourites = (await fetchHubOpenClozeFavourites()).map(prepareOpenClozeItem);
      if (!favourites.length) {
        toast("No favourite open cloze items yet.");
        return;
      }
      await logHubOpenClozeReviewLoaded({
        mode: "favourites",
      ...
      console.error("[HubOpenClozeTrainer] load favourites failed", error);
      toast("Could not load favourite open cloze items.");
    }
  }

  async function loadRecentMistakes() {
    try {
      const mistakes = (await fetchHubOpenClozeMistakes()).map(prepareOpenClozeItem);
      const deduped = [];
      const seen = new Set();
      ...
      if (!deduped.length) {
        toast("No recent open cloze mistakes yet.");
        return;
      }
      ...
      console.error("[HubOpenClozeTrainer] load mistakes failed", error);
      toast("Could not load recent open cloze mistakes.");
    }
  }
  ...
  async function handleSubmit(index) {
    ...
    if (isCorrect && (!hadIncorrectAttempt || currentSetMode === "mistakes")) {
      try {
        await clearHubOpenClozeMistakes(challenge.itemId);
      } catch (error) {
        console.error("[HubOpenClozeTrainer] clear mistakes failed", error);
      }
      return;
    }
    ...
    try {
      ...
      console.error("[HubOpenClozeTrainer] record mistake failed", error);
    }
  }
  ...
  async function toggleFavourite(challenge) {
    ...
    try {
      if (isFavourite) {
        await removeHubOpenClozeFavourite(challenge.itemId);
        ...
        await saveHubOpenClozeFavourite({
          itemId: challenge.itemId,
          gappedSentence: challenge.gappedSentence || "",
      ...
      console.error("[HubOpenClozeTrainer] favourite toggle failed", error);
  ...
  async function handleSendReport(index) {
    ...
    try {
      ...
      console.error("[HubOpenClozeTrainer] report failed", error);
  ...
  return (
    <div className="hub-keyword-page">
      <Seo
        title="Open Cloze | Seif Hub"
        description="Practise open cloze inside the Seif English Hub."
      />
      ...
      <div className="hub-keyword-shell">
        ...
        <header className="hub-keyword-header">
          ...
          <h1>Open Cloze</h1>
          <p>
            Complete each gap with the word that fits naturally in the sentence. This version now
            includes favourites, recent-mistakes review, reporting, and the same filter flow as the
            other Use of English trainers.
          </p>
        </header>
        ...
        <section className="hub-keyword-card">
          <div className="hub-keyword-filters">
            ...
            <div className="field-block">
              <label className="field-label" htmlFor="oc-primary-tag">Primary tag</label>
              <select
                id="oc-primary-tag"
                value={primaryTag}
                onChange={(event) => {
            ...
            <div className="field-block">
              <label className="field-label" htmlFor="oc-secondary-tag">Secondary tag</label>
              <select
                id="oc-secondary-tag"
                value={secondaryTag}
              ...
              {result && (
                <div className={`hub-keyword-feedback ${result.isCorrect ? "is-correct" : "is-wrong"}`}>
                  ...
                  {!result.isCorrect && !!result.bestFeedback?.letters?.length && (
                    <div className="hub-opencloze-letter-feedback">
                      {result.bestFeedback.letters.map((item, tokenIndex) => (
                        <span
                          key={`${item.className}-${item.token}-${tokenIndex}`}
                          className={`hub-opencloze-letter ${item.className}`}
                        >
                          {item.token}
                        </span>
                      ))}
      ...
      <style>{`
        ...
        .hub-opencloze-letter.correct {
          background: rgba(52, 211, 153, 0.18);
          border-color: rgba(52, 211, 153, 0.32);
          color: #d1fae5;
        }
        .hub-opencloze-letter.wrong {
          background: rgba(248, 113, 113, 0.18);
          border-color: rgba(248, 113, 113, 0.32);
          color: #ffe4e6;
        }
        .hub-opencloze-letter.missing {
          background: rgba(250, 204, 21, 0.16);
          border-color: rgba(250, 204, 21, 0.3);
          color: #fef3c7;
        }
        .hub-opencloze-letter.extra {
