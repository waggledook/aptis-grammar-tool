...
import { speakingPart1Key } from "../../data/packKey/speakingPart1";
        ...
        <div className="pk-body">
          ...
          {item.samples?.map((s, idx) => (
            <div key={s.label ?? idx} className="pk-sample">
              ...
              {s.answer && (
                <div className="pk-expl" style={{ fontSize: ".98rem", color: "rgba(255,255,255,.88)" }}>
                  {s.answer}
                </div>
              )}
            ...
            <div className="pk-errorGrid">
              {item.errors.map((e, idx) => (
                <div key={idx} className="pk-errorCard">
                  ...
                  {e.notes && <div className="pk-expl" style={{ marginTop: ".25rem" }}>{e.notes}</div>}
                </div>
              ))}
            </div>
          ) : null}
            ...
            <div className="pk-lineList" style={{ gap: ".7rem" }}>
              {item.exams.map((exam) => (
                <div key={exam.examId ?? exam.title} className="pk-exam">
                  ...
                  {exam.reminder && <p className="pk-sectionMeta" style={{ marginTop: ".35rem" }}>{exam.reminder}</p>}
...
export default function SpeakingPart1Key() {
  const sections = useMemo(() => speakingPart1Key.sections || [], []);
  ...
  return (
    <div className="packkey-page">
      ...
      <Seo
        title="Speaking Part 1 Key | Seif Aptis Trainer"
        description="Answer key for Speaking Part 1 (Aptis General) in the Seif Aptis Trainer pack, including sample answers and explanations."
        canonical="https://aptis-trainer.beeskillsenglish.com/pack-key/speaking-part-1"
      />
      ...
      <div className="panel" style={{ maxWidth: 980, margin: "0 auto" }}>
        ...
        <h1 className="pk-title">{speakingPart1Key.title} — Key</h1>
        <p className="pk-subtitle">
          Detailed answer key for Speaking Part 1. This key can include suggested phrases, corrections,
          and sample answers for the exam practice tasks.
        </p>
