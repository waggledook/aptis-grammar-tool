...
import { getAptisListeningStrategyGuide } from "./aptisListeningStrategyGuideData.js";
import "../../reading/aptisReadingStrategyGuide.css";

const FACT_ICONS = [Headphones, ListChecks, Volume2];
...
export default function AptisListeningStrategyGuide({ partNumber: partNumberOverride = "" }) {
  const navigate = useNavigate();
  const { partNumber: routePartNumber = "" } = useParams();
  const partNumber = partNumberOverride || routePartNumber;
  const guide = getAptisListeningStrategyGuide(partNumber);
  ...
  useEffect(() => {
    ...
    logStrategyGuideViewed({
      skill: "listening",
      part: guide.number,
      guideId: `listening_part${guide.number}_strategy_guide`,
  ...
  if (!guide) return <Navigate to="/listening" replace />;
  ...
  return (
    <main className="aptis-strategy-page aptis-listening-strategy-page">
      <Seo
        title={`Aptis Listening Part ${guide.number}: ${guide.title} Strategy Guide | Seif Aptis Trainer`}
      ...
      <header className="aptis-strategy-hero">
        <p className="aptis-strategy-kicker">Aptis Listening Part {guide.number}</p>
        <h1>{guide.title}</h1>
      ...
      <section className="aptis-strategy-facts" aria-label={`Part ${guide.number} essentials`}>
        {guide.facts.map((fact, index) => {
          const FactIcon = FACT_ICONS[index] || Headphones;
          return (
