...
function StepActions({ stepIndex, setStepIndex, canContinue = true }) {
  return (
    <footer className="prep-step-actions">
      ...
      <button
        type="button"
        disabled={!canContinue || stepIndex === relationshipPreparationSteps.length - 1}
        onClick={() => setStepIndex((index) => Math.min(relationshipPreparationSteps.length - 1, index + 1))}
      >
        Continue →
      </button>
    </footer>
...
function PhraseActivation({ ratings, setRatings, onComplete, stepIndex, setStepIndex }) {
  const [cardIndex, setCardIndex] = useState(0);
  const [flipped, setFlipped] = useState(false);
  const card = relationshipPhraseCards[cardIndex];
  const finished = Object.keys(ratings).length === relationshipPhraseCards.length;
  ...
  function rateCard(rating) {
    ...
    setFlipped(false);
    if (Object.keys(nextRatings).length === relationshipPhraseCards.length) {
      onComplete();
      return;
    }
    const nextUnrated = relationshipPhraseCards.findIndex((item, index) => index > cardIndex && !nextRatings[item.id]);
    setCardIndex(nextUnrated >= 0 ? nextUnrated : relationshipPhraseCards.findIndex((item) => !nextRatings[item.id]));
  }
  ...
  if (finished) {
    return (
      <section className="prep-activity-card prep-complete-card">
        ...
        <p>You recalled {relationshipPhraseCards.length - needsPractice} of {relationshipPhraseCards.length} confidently. {needsPractice ? `${needsPractice} can come back in your next review.` : "Everything felt familiar."}</p>
        {needsPractice ? (
          <button
            className="workshop-secondary"
            type="button"
            onClick={() => {
              const first = relationshipPhraseCards.findIndex((item) => ratings[item.id] === "again");
        ...
        <StepActions stepIndex={stepIndex} setStepIndex={setStepIndex} />
      </section>
    );
  }
    ...
    <section className="prep-activity-card">
      <div className="prep-activity-heading">
        <div>
          ...
          <h2>Which relationship phrase fits?</h2>
        </div>
        <strong>{Object.keys(ratings).length + 1} / {relationshipPhraseCards.length}</strong>
      </div>
      ...
      <StepActions stepIndex={stepIndex} setStepIndex={setStepIndex} canContinue={false} />
    </section>
  );
}

function ContextChoice({ answers, setAnswers, onComplete, stepIndex, setStepIndex }) {
  const [questionIndex, setQuestionIndex] = useState(0);
  const question = relationshipContextQuestions[questionIndex];
  const selected = answers[question.id];
  const finished = Object.keys(answers).length === relationshipContextQuestions.length;
  const score = relationshipContextQuestions.filter((item) => answers[item.id] === item.answer).length;

  function choose(option) {
    if (selected) return;
    const nextAnswers = { ...answers, [question.id]: option };
    setAnswers(nextAnswers);
    if (Object.keys(nextAnswers).length === relationshipContextQuestions.length) onComplete();
  }

  if (finished && questionIndex === relationshipContextQuestions.length - 1 && selected) {
    return (
      <section className="prep-activity-card prep-complete-card">
        <span className="prep-card-icon" aria-hidden="true">{score >= 5 ? "✓" : "↻"}</span>
        <h2>{score} / {relationshipContextQuestions.length} correct</h2>
  ...
  return (
    <section className="prep-activity-card">
      <div className="prep-activity-heading">
        ...
        <strong>{questionIndex + 1} / {relationshipContextQuestions.length}</strong>
      </div>
      ...
      <footer className="prep-step-actions">
        ...
        <button type="button" disabled={!selected} onClick={() => setQuestionIndex((index) => Math.min(relationshipContextQuestions.length - 1, index + 1))}>Next question →</button>
      </footer>
    </section>
  );
}

function AnswerBuilder({ selections, setSelections, onComplete, stepIndex, setStepIndex }) {
  const ready = relationshipAnswerBuilder.every((group) => selections[group.id]);
  ...
  return (
    <section className="prep-activity-card">
      ...
      <p className="prep-lead">Imagine the question is: <strong>“Tell me about someone you are close to.”</strong> Choose one useful stem at each stage.</p>
      <div className="prep-builder-grid">
        {relationshipAnswerBuilder.map((group) => (
      ...
      <aside className={`prep-answer-plan ${ready ? "is-ready" : ""}`}>
        ...
        {relationshipAnswerBuilder.map((group) => <p key={group.id}>{selections[group.id] || <em>Choose a phrase above…</em>}</p>)}
        {ready ? <small>Say this framework aloud and complete each ending with your own ideas.</small> : null}
      </aside>
      <StepActions stepIndex={stepIndex} setStepIndex={setStepIndex} canContinue={ready} />
    </section>
  );
}

function PhotoRehearsal({ topic, onComplete, stepIndex, setStepIndex }) {
  const task = topic.parts[2].tasks[4];
  const [questionIndex, setQuestionIndex] = useState(1);
  const [secondsLeft, setSecondsLeft] = useState(45);
  const [running, setRunning] = useState(false);
  const [checks, setChecks] = useState({ answer: false, reason: false, phrase: false });
  ...
  useEffect(() => {
    setRunning(false);
    setSecondsLeft(45);
    setChecks({ answer: false, reason: false, phrase: false });
  }, [questionIndex]);
  ...
  return (
    <section className="prep-activity-card">
      ...
      <div className="prep-rehearsal-layout">
        ...
        <div>
          ...
          <details className="prep-language-help">
            ...
            <p><strong>Useful phrases:</strong> keep in touch · get on well · rely on · grow apart</p>
            <p><strong>Develop:</strong> The main reason is… · For example… · In general, I think…</p>
          </details>
      ...
      <button className="prep-finish-button" type="button" disabled={checkedCount < 3} onClick={finish}>Complete preparation ✓</button>
      <StepActions stepIndex={stepIndex} setStepIndex={setStepIndex} />
    </section>
  );
}

export default function RelationshipsPreparation({ topic, user }) {
  const navigate = useNavigate();
  const storageKey = useMemo(() => `speaking-workshop-prep:${user?.uid || "local"}:${topic.id}:v1`, [topic.id, user?.uid]);
  ...
  const currentStep = relationshipPreparationSteps[stepIndex];
  const allComplete = completedSteps.length === relationshipPreparationSteps.length;
  ...
  return (
    <div className="workshop-prep-shell">
      ...
      <section className="prep-progress-summary" aria-label={`${completedSteps.length} of ${relationshipPreparationSteps.length} preparation steps complete`}>
        <div><strong>{completedSteps.length} / {relationshipPreparationSteps.length}</strong><span>steps complete</span></div>
        <div className="prep-progress-track" aria-hidden="true"><span style={{ width: `${(completedSteps.length / relationshipPreparationSteps.length) * 100}%` }} /></div>
      ...
      <nav className="prep-step-nav" aria-label="Preparation steps">
        {relationshipPreparationSteps.map((step, index) => (
      ...
      {currentStep.id === "phrases" ? <PhraseActivation ratings={ratings} setRatings={setRatings} onComplete={() => completeStep("phrases")} stepIndex={stepIndex} setStepIndex={setStepIndex} /> : null}
      {currentStep.id === "context" ? <ContextChoice answers={contextAnswers} setAnswers={setContextAnswers} onComplete={() => completeStep("context")} stepIndex={stepIndex} setStepIndex={setStepIndex} /> : null}
      {currentStep.id === "builder" ? <AnswerBuilder selections={builderSelections} setSelections={setBuilderSelections} onComplete={() => completeStep("builder")} stepIndex={stepIndex} setStepIndex={setStepIndex} /> : null}
      {currentStep.id === "rehearsal" ? <PhotoRehearsal topic={topic} onComplete={() => completeStep("rehearsal")} stepIndex={stepIndex} setStepIndex={setStepIndex} /> : null}
    </div>
  );
}
