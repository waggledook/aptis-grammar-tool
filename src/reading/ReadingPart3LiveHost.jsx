...
import { ReadingPart3Comments, ReadingPart3FullReview } from "./ReadingPart3LiveShared.jsx";
...
function getAnswers(player, task) {
  if (!player?.readingPart3Submission || !task) return {};
  return player.readingPart3Submission.taskId === task.id ? player.readingPart3Submission.answers || {} : {};
}
...
export default function ReadingPart3LiveHost({ user }) {
  ...
  const activityDetails = {
    ...
    activityTitle: game?.title || `Aptis Reading Part 3 · ${task?.title || "Live session"}`,
    part: 3,
    taskId: task?.id || null,
  ...
  async function beginReview() {
    ...
    if (incomplete > 0 && !window.confirm(`${incomplete} student${incomplete === 1 ? " has" : "s have"} not answered all seven questions. Saved partial answers will still appear. Start the review now?`)) return;
  ...
  if (loading) return <main className="rp1-live-page"><p>Loading Reading Part 3 room…</p></main>;
  ...
  if (!isHost) return <main className="rp1-live-page"><h1>Reading Part 3 live</h1><p>You are not the host of this session.</p></main>;

  return (
    <main className="rp1-live-page">
      <Seo title={`Host Reading Part 3: ${task.title}`} description="Teacher-controlled Aptis Reading Part 3 classroom session." />
      <header className="rp1-live-header"><div><p>Aptis Reading Part 3 · Live classroom</p><h1>{task.title}</h1></div><div className="rp1-live-phase"><span>{PHASE_LABELS[phase]}</span><strong>{phase === "task" ? `${completedCount}/${players.length} complete` : `${players.length} joined`}</strong></div></header>
      ...
      {phase === "review" ? (
        <section className="rp1-live-stage">
          <div className="rp1-live-status"><CheckCircle2 size={21} /><div><strong>Reviewing all seven matches</strong><span>{whyRevealed ? "Evidence and explanations are now visible to everyone." : "Answers and class responses are visible; evidence and explanations remain hidden."}</span></div></div>
          <ReadingPart3Comments task={task} />
          <ReadingPart3FullReview players={players} showWhy={whyRevealed} task={task} />
...
function HostReport({ players, task }) {
  ...
  return <section className="rp1-live-stage rp1-live-report"><CheckCircle2 size={44} /><span>Session complete</span><h2>Class average: {average.toFixed(1)} / 7</h2><p>{completed} of {players.length} students answered all seven questions.</p></section>;
}
