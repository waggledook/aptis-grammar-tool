...
export default function ReadingAssignButton({
  user,
  activityId,
  activityLabel,
  routePath,
  ...
  useEffect(() => {
    ...
    async function loadStudents() {
      ...
      try {
        ...
        console.error("[ReadingAssignButton] student load failed", error);
        if (alive) {
          setStudents([]);
  ...
  async function handleAssign() {
    ...
    if (!targetStudentIds.length) {
      toast("Choose at least one student for this reading task.");
      return;
    }
    ...
    try {
      ...
      toast("Reading task assigned.");
      close();
    } catch (error) {
      console.error("[ReadingAssignButton] assignment failed", error);
      toast("Could not assign that reading task.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <>
      <style>{`
        ...
        .reading-assign-head h3,
        .reading-assign-head p {
          margin: 0;
        }
        ...
        .reading-assign-field,
        .reading-assign-students {
          display: grid;
          gap: 0.45rem;
          margin-bottom: 0.9rem;
        }

        .reading-assign-field span,
        .reading-assign-label {
          font-size: 0.88rem;
          color: #a9b7d1;
        }

        .reading-assign-field select,
        .reading-assign-field textarea {
          width: 100%;
          border-radius: 12px;
        ...
        .reading-assign-student-item em {
          margin-left: auto;
          color: #8fb6ff;
          font-style: normal;
          font-size: 0.84rem;
      ...
      {open ? (
        <div className="reading-assign-overlay" onClick={close}>
          <div className="reading-assign-modal" onClick={(event) => event.stopPropagation()}>
            <div className="reading-assign-head">
              <div>
                <h3>Assign reading task</h3>
                <p>{activityLabel}</p>
              </div>
            ...
            {loading ? (
              ...
              <>
                <div className="reading-assign-grid">
                  <label className="reading-assign-field">
                    <span>Assign to</span>
                ...
                {targetMode === "selected" ? (
                  <div className="reading-assign-students">
                    <span className="reading-assign-label">Students</span>
                    {students.length ? (
                      <div className="reading-assign-student-list">
                        {students.map((student) => (
                          <label key={student.id} className="reading-assign-student-item">
                            <input
                              type="checkbox"
                ...
                <div className="reading-assign-actions">
