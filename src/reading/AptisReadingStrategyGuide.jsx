...
const FACT_ICONS = [FileCheck2, Target, Lightbulb];
...
export default function AptisReadingStrategyGuide() {
  ...
  useEffect(() => {
    ...
    saveReadingProgress(guide.progressId, `part${guide.number}`).catch((error) => {
      completionSavedRef.current = false;
      console.warn(`[Aptis Reading Part ${guide.number}] guide completion save failed`, error);
    });
  }, [answeredCount, guide]);

  if (!guide) return <Navigate to="/reading" replace />;
  ...
  return (
    <main className="aptis-strategy-page">
      <Seo
        title={`Aptis Reading Part ${guide.number}: ${guide.title} Strategy Guide | Seif Aptis Trainer`}
      ...
      <header className="aptis-strategy-hero">
        <p className="aptis-strategy-kicker">Aptis Reading Part {guide.number}</p>
        <h1>{guide.title}</h1>
      ...
      <section className="aptis-strategy-facts" aria-label={`Part ${guide.number} essentials`}>
        {guide.facts.map((fact, index) => {
          const FactIcon = FACT_ICONS[index] || Target;
          return (
