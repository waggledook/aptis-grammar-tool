...
function renderReferenceText(paragraph, paragraphIndex, highlights = []) {
...
function SourceCard({ title, paragraphs, source = "", referenceHighlights = [] }) {
  if (!source) return <article className="ote-reading-source-card"><header><h2>{title}</h2></header>{paragraphs.map((paragraph, paragraphIndex) => <p key={paragraph}>{renderReferenceText(paragraph, paragraphIndex, referenceHighlights)}</p>)}</article>;
