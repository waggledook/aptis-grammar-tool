      ...
      {phase === "ready" ? <ReadyCard user={user} onStart={startPractice} isTextMatching={isTextMatching} /> : phase === "active" || phase === "review" ? <article className="ote-practice-task-card ote-reading-matching-task">
        ...
        <div className="ote-reading-matching-layout"><div className="ote-reading-question-column"><div className="ote-reading-match-list">{questions.map((item, index) => {
          ...
          return <article className={`ote-reading-match-item ${expanded ? "is-open" : ""} ${answer ? "is-answered" : ""} ${reviewed ? (isCorrect ? "is-correct" : "is-wrong") : ""}`} key={item.id}>
            ...
            {expanded ? <div className="ote-reading-match-options">{!isTextMatching ? <p className="ote-reading-match-profile">{item.prompt}</p> : null}{Object.entries(options).map(([choice, option]) => <button key={choice} type="button" aria-pressed={answer === choice} className={`${answer === choice ? "is-selected" : ""} ${reviewed && choice === item.answer ? "is-answer" : ""} ${reviewed && answer === choice && !isCorrect ? "is-incorrect" : ""}`} disabled={reviewed} onClick={() => chooseAnswer(item.id, choice)}>{choice}. {option.title}</button>)}{reviewed ? <div className={`ote-reading-item-feedback ${isCorrect ? "is-correct" : "is-wrong"}`}><strong>{isCorrect ? "Correct." : "Not quite."}</strong><p>{item.feedback}</p></div> : null}</div> : null}
          </article>;
