...
export async function markOteTrainingProgress(details = {}) {
  ...
  [
    ...
    ["evidenceScore", details.evidenceScore],
    ["evidenceTotal", details.evidenceTotal],
    ["matchScore", details.matchScore],
    ["diagnosisScore", details.diagnosisScore],
...
export async function fetchOteTrainingProgressDetails(uid) {
  ...
  return snap.docs.reduce((acc, entry) => {
    ...
    const detail = {
      ...
      evidenceScore: typeof data.evidenceScore === "number" ? data.evidenceScore : null,
      evidenceTotal: typeof data.evidenceTotal === "number" ? data.evidenceTotal : null,
  ...
  return snap.docs
    .map((entry) => {
      ...
      return {
        ...
        evidenceTotal: data.evidenceTotal ?? null,
        matchScore: data.matchScore ?? null,
        diagnosisScore: data.diagnosisScore ?? null,
        reason: data.reason || "",
...
export async function logReadingPart4Completed({ taskId, source = "AptisPart4", progressPart = "part4" }) {
